...
* **用量扫描磁盘持久化缓存（Persistent Cache Across Restarts）**：
  * 引入跨进程重启的用量数据磁盘持久化缓存机制，二次启动与重载时直接命中缓存，恢复时间从数十秒级缩短至毫秒级。
* **活跃会话内存快照直接复用**：
  * 统计分析时优先复用宿主活跃会话的内存快照（Live Session Snapshots），大幅减轻磁盘重复 I/O 读写压力。
* **DSH 0.1.2-rc.1 兼容性与 CI 加固**：
  * 适配 DSH 0.1.2 新版会话快照与用量数据结构，恢复全量统计兼容性；
  * 新增 GitHub Actions 自动化 CI 构建与测试流水线。
