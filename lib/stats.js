...
/**
 * Decode one parsed JSONL line into the session event(s) it stores.
 *
 * In DSH <= 0.1.2, assistant chunks were packed into text-chunks,
 * reasoning-chunks, and tool-call-chunks rows. DSH 0.1.5 removed packed chunk
 * rows and writes native SessionEvents directly. We unpack packed rows inline
 * for historical session logs and pass native events through.
 */
export function decodeStorageRecord(value) {
  ...
  const events = [];
  let time = typeof value.time0 === "number" ? value.time0 : undefined;
  for (let k = 0; k < members.length; k++) {
    if (k > 0 && typeof time === "number") {
      time += Number(dt[k - 1]) || 0;
    }
    let chunk;
    switch (type) {
      case "text-chunks":
      ...
      case "tool-call-chunks":
        chunk = {
          ...
          index: data?.index,
          id: data?.id,
          ...(data && Object.hasOwn(data, "name") ? { name: data.name } : {}),
    ...
    events.push({
      ...
      ...(typeof value.seq0 === "number" ? { seq: value.seq0 + k } : {}),
      ...(time !== undefined ? { time } : {}),
      data: {
        turn: data?.turn,
        step: data?.step,
  ...
  try {
    ...
    for await (const text of frameTextsAsync(buffer)) {
      for (const line of text.split("\n")) {
        ...
        const stored = parseEvent(trimmed);
        if (stored === null) continue;
        // Historical DSH persistence (<= 0.1.2) packed long assistant/chunk
        // runs into text-chunks/reasoning-chunks/tool-call-chunks rows. DSH 0.1.5
        // emits native events directly. Decode those rows inline before folding
        // so performance timestamps see the same event stream as a live
        // Session snapshot.
...
export function buildStats(calls, pricing, defaultPricing, options = {}) {
  ...
  for (const sample of calls) {
    ...
    // Keep same-named models from different providers separate; their rates,
    // latency and provider labels are not interchangeable.
    const modelKey = `${sample.provider ?? ""}\u0000${model}`;
    ...
    const sessionModelKey = `${sessionId}\u0000${sample.provider ?? ""}\u0000${model}`;
    ...
    if (perf !== undefined && perf !== null) {
      const perfKey = `${sample.provider ?? ""}\u0000${model}`;
      let perfBucket = byModelPerf.get(perfKey);
      if (perfBucket === undefined) {
        perfBucket = { model, provider: sample.provider ?? null, ttft: [], tps: [], latency: [] };
        byModelPerf.set(perfKey, perfBucket);
      }
