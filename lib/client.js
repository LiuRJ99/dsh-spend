...
`;
		...
		function SummaryCards({ t, totals, callCount, sessionsScanned, currency, rate, billing, budget, overview }) {
			...
			const formatBilling = (value) => formatCostIn(value, billingCurrency, currency, rate);
			const budgetCurrency = budget?.currency ?? "USD";
			const formatBudget = (value) => formatCostIn(value, budgetCurrency, currency, rate);
			const hasBilling = typeof monthly === "number" && Number.isFinite(monthly);
			const composition = (billing?.parts ?? [])
				.map((part) => `${part.provider} ${formatCostIn(part.amount, part.currency ?? billingCurrency, currency, rate)}${part.kind === "subscription" ? t("plan.perMonth") : ""}`)
				.join(" + ");
			...
			return react_jsx_runtime.jsx("div", {
				...
				children: [
					react_jsx_runtime.jsx(Card, {
						...
						cost: true,
						value: hasBilling ? `${formatBilling(monthly)}${t("plan.perMonth")}` : formatCost(totals.cost, currency, rate),
					...
					hasBudget
						? react_jsx_runtime.jsx(Card, {
							...
							cost: true,
							value: `${formatBudget(budget.monthly)} · ${Math.round(budget.pct)}%`,
							title: `${t("budget.used")} ${formatBudget(budget.used)} · ${t("budget.remaining")} ${formatBudget(budget.remaining)}${budget.pct >= 100 ? ` · ${t("budget.exceeded")}` : ""}`,
						})
						: null,
					activeDays > 0
		...
		function TimeCurveChart({ hours, currency, rate, t }) {
			...
			const safeHover = hover === null || hover < 0 || hover >= visible.length ? null : hover;
			const hovered = safeHover === null ? null : visible[safeHover];
			const hoverX = safeHover === null ? 0 : xAt(safeHover);
						...
						children: [
							...
							safeHover !== null
								? [
									...
									series.map((s) => react_jsx_runtime.jsx("circle", { cx: hoverX, cy: yAt(visible[safeHover][s.key] ?? 0), r: 3, fill: s.color, stroke: "var(--dsw-alias-bg-base,#ffffff)", strokeWidth: 1.5 }, `dot-${s.key}`)),
								]
								: null,
						],
					}),
...
.mono{font-family:ui-monospace,SFMono-Regular,Menlo,monospace;font-size:11px}
				...
				children: [
					react_jsx_runtime.jsx(Section, {
						...
						children: react_jsx_runtime.jsx(BreakdownTable, {
							...
							rowKey: (row) => `${row.sessionId}-${row.provider ?? "?"}-${row.model}`,
							renderRow: (row) => [
		...
		function UsageStatsWidget({ t, query }) {
			...
			const refreshAbort = react.useRef(null);
			const refresh = react.useCallback(() => {
				...
				setLoading(true);
				const request = query(() => requestedCwd)(controller.signal);
			...
			// Abort host RPCs and close the auxiliary window when the widget is
			// unloaded, so a removed plugin cannot retain network work or a stale
			// details document.
			react.useEffect(() => () => {
				refreshAbort.current?.abort();
				const win = detailsWin.current;
				if (win !== null && !win.closed) win.close();
				detailsWin.current = null;
			}, []);
			...
			// Re-query immediately when the workspace filter changes. Clear the
			// previous scope first so a slow/failed request cannot present old
			// workspace totals under the new selector.
			react.useEffect(() => {
				setData(null);
				setError(null);
				setLastUpdated(null);
