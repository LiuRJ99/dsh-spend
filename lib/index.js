...
usageEndpoints;
    ...
    constructor(ctx, config = {}) {
      ...
      // Pricing rows are USD-denominated. `currency` is a display preference;
      // keep the host snapshot's accounting currency explicit so CNY cannot
      // accidentally be applied to already-USD aggregate costs.
      this.displayCurrency = this.currency === "CNY" ? "CNY" : "USD";
    ...
    async compute(signature, live, cwd) {
      ...
      // token providers their USD-denominated 30-day estimate. Aggregate by
      // canonical provider so aliases cannot duplicate a subscription fee.
      const billingParts = [];
      const tokenBilling = new Map();
      ...
      for (const row of stats.byProvider) {
        ...
        const canonical = normalizeProvider(row.provider);
        const plan = mergedPlans.find((candidate) => normalizeProvider(candidate.provider) === canonical);
        if (plan?.type === "code") {
          if (!subscriptionProviders.has(canonical)
            && plan.subscription?.amount !== undefined && plan.subscription.amount !== null) {
            subscriptionProviders.add(canonical);
            billingParts.push({
              provider: plan.provider ?? row.provider,
              kind: "subscription",
        ...
        // monthly counterpart of subscription fees), always in USD because
        // pricing rows are USD-denominated regardless of display preference.
        const amount = stats.recentCostByProvider?.[row.provider] ?? row.cost;
        const previous = tokenBilling.get(canonical);
        if (previous === undefined) {
          tokenBilling.set(canonical, { provider: row.provider, amount });
        } else {
          previous.amount += amount;
        }
      }
      for (const part of tokenBilling.values()) {
        billingParts.push({ ...part, kind: "token", currency: "USD", period: "30d" });
      }
      // Explicit Code plans still contribute their subscription fee when the
      // current workspace has no calls from that provider.
      for (const plan of mergedPlans) {
        const canonical = normalizeProvider(plan.provider);
        if (plan.type !== "code" || subscriptionProviders.has(canonical)
          || plan.subscription?.amount === undefined || plan.subscription.amount === null) continue;
        subscriptionProviders.add(canonical);
        billingParts.push({
      ...
      const billingTotal = billingParts.every((part) => part.currency === "USD")
      ...
      // Budgets follow the configured display currency, while billing totals
      // are USD. Normalize the configured CNY budget back to USD once here;
      // the client then converts the explicit USD fields for presentation.
      const budgetUsd = this.monthlyBudget !== null && this.displayCurrency === "CNY"
        ? this.monthlyBudget / (Number(rates.CNY) > 0 ? rates.CNY : this.usdCnyRate)
        : this.monthlyBudget;
      let budget = null;
      if (billingTotal !== null && budgetUsd !== null && budgetUsd > 0) {
        budget = {
          currency: "USD",
          monthly: budgetUsd,
          used: billingTotal,
          remaining: Math.max(0, budgetUsd - billingTotal),
          pct: Math.min(100, (billingTotal / budgetUsd) * 100),
        };
      }
      ...
      const snapshot = {
        ...
        // Aggregate token costs and budgets are always USD; the client may
        // convert them to `displayCurrency` using the served rates.
        currency: "USD",
