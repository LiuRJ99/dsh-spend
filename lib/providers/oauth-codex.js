...
/** Default rate-limit endpoint; `usageEndpoints` may override it. */
...
async function codexWham(io, accessToken, accountId, endpoint) {
  ...
  const res = await request(endpoint?.url ?? CODEX_USAGE_URL, {
    headers,
...
export async function codexFetch(io, endpoint) {
  ...
  let json;
  try {
    json = await codexWham(io, auth.accessToken, auth.accountId, endpoint);
  } catch (error) {
    if (error?.code !== "AUTH" || typeof auth.refreshToken !== "string") throw error;
    const refreshed = await codexRefresh(io, auth.refreshToken);
    json = await codexWham(io, refreshed.access_token, auth.accountId, endpoint);
  }
