...
/** Default internal usage endpoint; `usageEndpoints` may override it. */
...
/** Locate a GitHub token: explicit override, env, gh CLI hosts file, then copilot CLI hosts file. */
async function copilotToken(io, endpoint) {
  if (typeof endpoint?.apiKeyEnv === "string" && endpoint.apiKeyEnv.length > 0) {
    const explicit = await io.resolveRef(endpoint.apiKeyEnv);
    if (typeof explicit === "string" && explicit.length > 0) return explicit;
    const fromEnv = io.env[endpoint.apiKeyEnv];
    if (typeof fromEnv === "string" && fromEnv.length > 0) return fromEnv;
    throw notLoggedIn(`not logged in: no GitHub token (${endpoint.apiKeyEnv})`);
  }
...
export async function copilotFetch(io, endpoint) {
  const token = await copilotToken(io, endpoint);
  const res = await request(endpoint?.url ?? COPILOT_USAGE_URL, {
    headers: {
